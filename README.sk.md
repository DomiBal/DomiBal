# Ahoj, ja som Dominika! 👋

🪪[...in English](README.md)

Som **Financial Risk & Data Analytics Specialist** s matematickým backgroundom (FMFI UK) a vášňou pre transformáciu surových dát na strategické insighty. V súčasnosti pôsobím popri práci v oblasti finančného riadenia rizík a Solvency II aj ako **AI Champion** a správca Power BI Premium workspaceov.

Aktuálne sa intenzívne posúvam smerom k **Data Science** a budujem si portfólio projektov, v ktorých spájam pokročilú analytiku s reálnym spoločenským a biznisovým dopadom.

---

### 🛠️ Tech Stack & Zručnosti
* **Programovanie & Dáta:** Python (Pandas, NumPy, Scikit-learn), SQL, Git
* **BI & Vizualizácia:** Power BI (Premium Workspace Management, DAX, reportovanie)
* **Doménová expertíza:** Finančné modelovanie, Solvency II, Riadenie rizík (Risk Management), Bankový, poistný a dôchodkový legislatívny background

---

### 🚀 Moje Data Science Projekty

#### 📊 [sk_pension_analysis](https://github.com/DomiBal/sk-pension-analysis.git)
* **Popis:** Komplexná analýza dôchodkových štatistík zo Sociálnej poisťovne. Projekt pokrýva kompletný dátový cyklus od automatizovaného sťahovania dát cez ich transformáciu v Pythone až po finálny auditovaný Power BI report.
* **Tech Stack:** Python, Power BI, Audit Logging.

#### 🧱 [lego_portfolio_analytics](https://github.com/DomiBal/lego-portfolio-analysis.git) *(Pripravuje sa)*
* **Popis:** Migrácia flat-file Excel databázy (zbierka 125 setov) do relačnej databázy (SQL) prepojená s Python API (Rebrickable/Brickset) na sledovanie trhovej hodnoty, analýzu návatnosti investícií (ROI) a predikciu budúcej ceny setov po EOL.
* **Tech Stack:** SQL (PostgreSQL/SQLite), Python (API, Scikit-learn), Power BI.

#### 🦓 [zebra_education_analytics](ODKAZ_NA_TVOJ_PROJEKT_4) *(V pláne)*
* **Popis:** Dátová analýza faktorov ovplyvňujúcich znevýhodnenie vo vzdelávaní detí z marginalizovaných komunít.
* **Tech Stack:** Python (Scikit-learn - Klasifikácia, Geopandas), Power BI (GIS mapping).

#### 🏠 [housing_affordability_stresstest](ODKAZ_NA_TVOJ_PROJEKT_3) *(V pláne)*
#### 📖 [books_database_and_something_else](ODKAZ_NA_TVOJ_PROJEKT_3) *(V pláne)*

---

### 🤝 Dobrovoľníctvo & Hodnoty
Verím, že dáta by mali pomáhať tam, kde je to najviac potrebné. Vo voľnom čase pôsobím ako dobrovoľná online doučovateľka detí a mladých ľudí v tínedžerskom veku (11 – 19 rokov) z vylúčených komunít v **OZ Cesta VON (program Zebra)**, kde im pomáhame znižovať znevýhodnenie vo vzdelávaní a rozvíjať ich osobný potenciál.

### 📫 Kde ma nájdete:
* **LinkedIn:** [dominika-balabánová](https://www.linkedin.com/in/dominika-balab%C3%A1nov%C3%A1-18344a162)
* **Email:** dominika.balabanova@gmail.com
